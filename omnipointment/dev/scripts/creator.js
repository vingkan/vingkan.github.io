...
//LOCAL IMPORTS
import {TimeGridModel, TimeGridView} from './views/timegrid';
...
export var CreatorGridView = TimeGridView.extend({
	events: {
		'click .remove-date': 'removeDateOnClick',
		'click .remove-time': 'removeTimeOnClick'
	},
	removeDateOnClick: function(e){
		var element = e.target;
		var date = element.id.split("-")[2];
		this.removeDateOption(date);
	},
	removeTimeOnClick: function(e){
		var element = e.target;
		var time = element.id.split("&")[1];
		this.removeTimeOption(time);
	},
	render: function(){
