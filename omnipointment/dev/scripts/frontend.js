...
function getDateOptionFromInput(idPath, errorMinder){
	var dateInput = document.getElementById(idPath);
...
export var Front = {
	...
	getUID: function(cb){
		...
		if (window.me === null || window.me === undefined || (window.me.get && window.me.get('uid') !== uid)) {
			window.me = new User({ id: uid });
			window.me.fetch({ success: cb || function () {} });
		}
		return uid;
	},
	loadUser: function(){
